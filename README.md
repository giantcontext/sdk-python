...
<!-- API_REFERENCE_START -->
186 methods across 32 resources.
...
List the blocks available for a content type
Returns the block types allowed for a content type (its palette), plus the shared styles schema every block accepts. Use this first to see what you can insert, then getBlock for a specific block's own fields. Scoped to the same palette the content-edit tools validate against.
...
```python
result = await gc.builder.get_block_manifest(
    content_type="value",
)
```
...
Get a block's full schema
Returns the JSON schema for a single block type plus its per-field authoring hints, scoped to (and validated against) the content type's palette. The styles object is identical for every block and is served by getBlockManifest instead of being repeated here. Follow this exactly when building block data for insertBlock/updateBlock.
...
Delete a block
Removes a block from a content tree and records a version. Returns the removed block. Non-destructive at the history level — the prior state is recoverable via restoreContentVersion.
...
Find an exact string inside content blocks
Literal, case-insensitive string search across the blocks of pages, posts, landings, KB articles and developer docs in a project. Returns the content and block that matched, the field path, the locale whose copy matched, and a snippet. Use searchSources instead when looking for material by meaning rather than by exact wording.
...
Update the developer portal landing page's metadata
...
Update a developer doc category's metadata
...
```python
result = await gc.other.update_developers_doc_category_meta(
    organization_id="uuid-organization",
    project_id="uuid-project",
    app_id="uuid-app",
    category_id="uuid-category",
...
```python
result = await gc.other.create_developers_doc_category(
    organization_id="uuid-organization",
    project_id="uuid-project",
    app_id="uuid-app",
    data={...},
...
Update a developer doc's metadata
...
```python
result = await gc.other.update_developers_doc_meta(
    organization_id="uuid-organization",
    project_id="uuid-project",
    app_id="uuid-app",
    doc_id="uuid-doc",
...
```python
result = await gc.other.create_developers_doc(
    organization_id="uuid-organization",
    project_id="uuid-project",
    app_id="uuid-app",
    data={...},
...
```python
result = await gc.other.create_kb_article(
    organization_id="uuid-organization",
    project_id="uuid-project",
    app_id="uuid-app",
    data={...},
...
```python
result = await gc.other.create_kb_category(
    organization_id="uuid-organization",
    project_id="uuid-project",
    app_id="uuid-app",
    data={...},
...
Search project sources by meaning
...
Update the landing page's metadata
...
Create blog post
Creates a new blog post. Requires a title. Optionally set slug, content blocks, excerpt, tags, featured image, SEO metadata, and publish status.
...
```python
result = await gc.website.list_website_urls(
    organization_id="uuid-organization",
    project_id="uuid-project",
    app_id="uuid-app",
)
